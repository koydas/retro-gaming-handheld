...
RetroPie is built on top of Raspberry Pi OS, which maintains the DispmanX graphics stack on 32-bit builds. fbcp-ili9341 was originally developed and tested against this environment and has well-documented community setups with ILI9341 panels on Pi Zero hardware. The display driver compatibility concern that put ADR-0001 in provisional status is resolved by this choice.
...
- ADR-0001 display decision is unblocked: fbcp-ili9341 on RetroPie + Raspberry Pi OS (32-bit) is a tested, community-documented path.
